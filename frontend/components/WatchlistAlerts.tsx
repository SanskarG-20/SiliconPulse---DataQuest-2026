      ...
      {alerts.length === 0 ? (
        ...
        <ul className="space-y-2">
          {alerts.slice(0, 5).map((a: any, i: number) => (
            <li key={`${a.title}-${i}`}>
              <button onClick={() => onCompanyClick(a.matched_company || a.company)} className="w-full text-left rounded-[10px] bg-[#050B1A] border border-[#1C3553]/40 hover:border-[#E8A253]/30 p-2.5 transition-colors group">
                <p className="text-[12px] font-semibold leading-snug text-[#E2E8F0] group-hover:text-white line-clamp-2">{a.title}</p>
