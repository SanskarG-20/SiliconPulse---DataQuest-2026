    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex flex-wrap gap-1.5">
          {companies.map((c) => (
            <span key={c} className="inline-flex items-center gap-1 rounded-full bg-[#050B1A] border border-[#1C3553]/60 pl-2.5 pr-1.5 py-1 text-[12px] font-semibold text-[#E2E8F0]">
              {c}
              <button
                onClick={() => setCompanies(companies.filter((x) => x !== c))}
                className="p-0.5 rounded-full text-[#475569] hover:text-white transition-colors"
                aria-label={`Remove ${c}`}
              >
                <X size={12} />
              </button>
        ...
        {companies.length < 4 && (
          <span className="inline-flex items-center gap-1.5">
            <input
              ...
              className="w-[130px] px-2.5 py-1 rounded-full bg-[#050B1A] border border-[#1C3553]/60 text-[12px] text-[#E2E8F0] placeholder:text-[#475569] focus:outline-none focus:border-[#22D3EE]/40"
              aria-label="Add company"
            />
            <button onClick={() => addCompany(input)} className="p-1.5 rounded-full bg-[#0E1E32] border border-[#1C3553] text-[#94A3B8] hover:text-white transition-colors" aria-label="Add">
              <Plus size={13} />
            </button>
          </span>
        )}
      ...
      {watchlist.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {watchlist.filter((w) => !companies.some((c) => c.toLowerCase() === w.toLowerCase())).slice(0, 6).map((w) => (
            <button key={w} onClick={() => addCompany(w)} disabled={companies.length >= 4} className="px-2 py-1 rounded-full bg-[#E8A253]/10 border border-[#E8A253]/15 text-[11px] font-medium text-[#E8A253] hover:bg-[#E8A253]/15 transition-colors disabled:opacity-40">
              + {w}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input
          ...
          className="flex-1 min-w-[180px] px-3 py-1.5 rounded-full bg-[#050B1A] border border-[#1C3553]/60 text-[12px] text-[#E2E8F0] placeholder:text-[#475569] focus:outline-none focus:border-[#22D3EE]/40"
      ...
      {loading && !data && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="h-[86px] rounded-[12px] bg-[#050B1A] border border-[#1C3553]/30 animate-pulse" />
          ))}
        </div>
      )}
        ...
        <div className="space-y-3">
              ...
              <button
                ...
                className={`text-left rounded-[12px] border p-3 transition-colors ${
                  leader?.company === r.company
                    ...
                    : 'border-[#1C3553]/40 bg-[#050B1A] hover:border-[#22D3EE]/25'
                }`}
              >
                ...
                <p className="display mt-1 text-[20px] font-bold tracking-[-0.02em] text-[#E8A253]">{r.signal_strength}<span className="text-[11px] font-medium text-[#64748B]">/100</span></p>
                ...
                {r.summary?.latest_title && (
                  <p className="mt-1.5 text-[11px] leading-snug text-[#94A3B8] line-clamp-2">{r.summary.latest_title}</p>
                )}
              </button>
            ))}
          </div>
          ...
          {(data.overlap?.shared_downstream?.length > 0 || data.overlap?.shared_upstream?.length > 0) && (
            <div className="rounded-[12px] border border-[#22D3EE]/15 bg-[#22D3EE]/[0.05] px-3.5 py-2.5">
              <p className="mono text-[10px] font-semibold tracking-[0.1em] text-[#22D3EE] mb-1.5">SHARED EXPOSURE</p>
              <div className="flex flex-wrap gap-1.5">
                {(data.overlap.shared_downstream || []).slice(0, 4).map((o: any) => (
                  <span key={`d-${o.company}`} className="px-2 py-1 rounded-full bg-[#050B1A] border border-[#1C3553]/40 text-[11px] text-[#CBD5E1]">
                ...
                {(data.overlap.shared_upstream || []).slice(0, 4).map((o: any) => (
                  <span key={`u-${o.company}`} className="px-2 py-1 rounded-full bg-[#050B1A] border border-[#1C3553]/40 text-[11px] text-[#CBD5E1]">
                    ↑ {o.company} <span className="mono text-[10px] text-[#475569]">via {o.shared_by.join(' + ')}</span>
                  </span>
                ))}
              </div>
          ...
          {data.comparison_report && (
            <div className="rounded-[12px] border border-[#1C3553]/40 bg-[#050B1A]/60 p-4">
