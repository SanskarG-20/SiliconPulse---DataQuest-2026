    ...
    <div className="min-h-screen flex flex-col text-[#F1F5F9] relative">
      ...
      <header className="h-[56px] border-b border-slate-200/70 dark:border-[#1C3553]/50 flex items-center justify-between px-4 md:px-6 bg-white/70 dark:bg-[#050B1A]/70 backdrop-blur-xl">
        <Link to="/" className="flex items-center gap-2.5">
          ...
          <span className="display text-[13px] font-bold tracking-[-0.02em] text-slate-900 dark:text-white">SILICON<span className="text-[#0284C7] dark:text-[#22D3EE]">PULSE</span></span>
          <span className="mono text-[9px] tracking-[0.14em] text-[#475569] border border-slate-300 dark:border-[#1C3553]/50 rounded-full px-2 py-0.5 bg-slate-100 dark:bg-[#0E1E32]">SHARED BRIEF</span>
        </Link>
      ...
      <main className="flex-1 px-4 md:px-6 py-8">
        <div className="max-w-[760px] mx-auto">
          {!brief && !missing && (
            <div className="rounded-[16px] border border-slate-200 dark:border-[#1C3553]/40 bg-white/60 dark:bg-[#0E1E32]/50 p-8 text-center mono text-[12px] text-[#64748B] animate-pulse">
          ...
          {missing && (
            <div className="rounded-[16px] border border-red-500/15 bg-red-500/[0.06] p-8 text-center">
              ...
              <Link to="/" className="mt-4 inline-block text-[12px] text-[#0284C7] dark:text-[#22D3EE] hover:underline">Back to home</Link>
            </div>
          )}
          {brief && (
            <div className="space-y-4">
              <p className="mono text-[10px] tracking-[0.12em] font-semibold text-[#0284C7] dark:text-[#22D3EE]">SHARED INTELLIGENCE REPORT</p>
              <h1 className="display text-[24px] font-bold tracking-[-0.02em] text-slate-900 dark:text-white leading-tight">“{brief.query}”</h1>
              <p className="mono text-[11px] text-[#64748B]">{brief.created_at ? new Date(brief.created_at).toLocaleString() : ''}</p>
              <div className="rounded-[16px] border border-slate-200 dark:border-[#1C3553]/50 bg-white/70 dark:bg-[#0E1E32]/50 p-5 md:p-6">
                <StrategicInsightReport data={brief.insight} />
              </div>
              {Array.isArray(brief.evidence) && brief.evidence.length > 0 && (
                <div className="rounded-[16px] border border-slate-200 dark:border-[#1C3553]/40 bg-white/60 dark:bg-[#0B1426]/60 p-5">
                  ...
                  <ul className="space-y-2">
                    {brief.evidence.slice(0, 10).map((e: any, i: number) => (
                      <li key={i} className="text-[13px] text-slate-600 dark:text-[#CBD5E1]">
                        <span className="font-semibold text-slate-900 dark:text-white">{e.title || 'Untitled'}</span>
